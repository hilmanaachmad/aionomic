...
Page.onReady = function() {
    ...
    //console.log(Variables.loggedInUserData.dataSet.user_supervisor_nik)
};
...
Page.dateFromChange = function($event, widget, newVal, oldVal) {
    if (Page.Variables.vmDelegationModel.dataSet.delExpDatetime) {
        if (Page.Variables.vmDelegationModel.dataSet.delAvailDatetime > Page.Variables.vmDelegationModel.dataSet.delExpDatetime) {
            Page.Variables.vmDelegationModel.dataSet.delAvailDatetime = null
            App.Actions.appNotification.setMessage('Delegation Date is invalid')
...
Page.dateToChange = function($event, widget, newVal, oldVal) {
    ...
    if (Page.Variables.vmDelegationModel.dataSet.delAvailDatetime) {
        if (Page.Variables.vmDelegationModel.dataSet.delAvailDatetime > Page.Variables.vmDelegationModel.dataSet.delExpDatetime) {
            Page.Variables.vmDelegationModel.dataSet.delExpDatetime = null
            App.Actions.appNotification.setMessage('Delegation Date is invalid')
            App.Actions.appNotification.setToasterClass('error')
            App.Actions.appNotification.getToasterDuration(5000)
            App.Actions.appNotification.invoke()
        ...
        var emailBody = `<html>
            ...
            <table style=" width: 560px;
              ...
              <td>
               ...
               <p style="color: #262626;">Please be informed that ${Page.App.Variables.loggedInUserData.dataSet.user_full_name} has delegated the budget module in the aio procure to pay application to ${Page.Variables.vmDelegationModel.dataSet.delToUserName} from ${Page.Variables.vmDelegationModel.dataSet.delAvailDatetime} until ${Page.Variables.vmDelegationModel.dataSet.delExpDatetime} for the following reasons: <br><b>${Page.Variables.vmDelegationModel.dataSet.delReason}</b></p> <br>
            	</td>
              </tr>
