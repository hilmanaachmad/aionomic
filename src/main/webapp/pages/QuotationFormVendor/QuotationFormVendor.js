...
Page.buttonSubmitClick = function($event, widget) {
    ...
    // data.rfqvBasePrice = Page.parseInt(data.rfqvBasePrice)
    // data.rfqvOtherCost = Page.parseInt(data.rfqvOtherCost)
    // data.rfqvPackingCost = Page.parseInt(data.rfqvPackingCost);
    // data.rfqvShippingHandling = Page.parseInt(data.rfqvShippingHandling)
    ...
    // dataRFQ.rfqModifiedBy = App.Variables.loggedInUserData.dataSet.username
    ...
    Page.Variables.vdbRfqVendor.updateRecord({
        ...
        Page.Widgets.spinner1.show = false
        App.Actions.goToPage_QuotationFormVendorDone.invoke()
    })
...
Page.calculationPackingCost = function() {
    // console.log(Page.formatCalaculate(Page.Widgets.textSubtotal.datavalue) + Page.formatCalaculate(Page.Widgets.textVatRate2.datavalue))
...
Page.button6_1Click = function($event, widget) {
    ...
    // data.amount = data.qty * data.unitPrice * Page.parseInt(data.kurs)

    data.amount = data.qty * data.unitPrice
    // data.amountAfterDiscount = data.amount - Math.round(data.amount * data.discount / 100)
    data.amountAfterDiscount = data.amount - (data.amount * data.discount / 100)
    console.log(data.amountAfterDiscount)
    Page.Variables.modelCalculation.dataSet.subtotal = data.amountAfterDiscount + Page.Variables.modelCalculation.dataSet.subtotal
...
Page.textVatRateKeyup = function($event, widget) {
    ...
    if (Page.Widgets.search3.datavalue.fromCurrency == 'IDR' || Page.Widgets.search3.datavalue.fromCurrency == 'JPY') {
        ...
        if (newVal.includes(",") && indexKe != (newVal.length - 1)) {
            ...
            console.log(Page.Variables.modelRfqVendor.dataSet)
        } else {
