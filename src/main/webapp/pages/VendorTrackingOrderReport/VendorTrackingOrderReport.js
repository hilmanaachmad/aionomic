...
Page.select9Change = function($event, widget, newVal, oldVal) {
    Page.Variables.vdbTrackingOrder.maxResults = newVal
    Page.Variables.vdbTrackingOrder.update()
};
...
Page.selectSearchByChange = function($event, widget, newVal, oldVal) {
    ...
    Page.Widgets.selectCompany.datavalue = ""
};
...
Page.toggleTableSort = function(field) {
    if (Page.Variables.vdbTrackingOrder.orderBy == field + " ASC") {
        Page.Variables.vdbTrackingOrder.orderBy = field + " DESC"
    } else if (Page.Variables.vdbTrackingOrder.orderBy == field + " DESC") {
        Page.Variables.vdbTrackingOrder.orderBy = ""
    } else {
        Page.Variables.vdbTrackingOrder.orderBy = field + " ASC"
    }
    Page.Variables.vdbTrackingOrder.invoke()
}

Page.showWidgetSearch = function() {
    if (Page.Widgets.selectSearchBy.datavalue == 3 || Page.Widgets.selectSearchBy.datavalue == 4 || Page.Widgets.selectSearchBy.datavalue == 5 || Page.Widgets.selectSearchBy.datavalue == '' || Page.Widgets.selectSearchBy.datavalue == undefined) {
        return true;
    } else {
        return false;
    }
...
Page.showWidgetDate = function() {
    if (Page.Widgets.selectSearchBy.datavalue == 1 || Page.Widgets.selectSearchBy.datavalue == 6 || Page.Widgets.selectSearchBy.datavalue == 7 || Page.Widgets.selectSearchBy.datavalue == 8) {
        return true;
    } else {
        return false;
    }
...
Page.container41Click = function($event, widget) {
    Page.toggleTableSort('receivedDate')
};

Page.container42Click = function($event, widget) {
    Page.toggleTableSort('companyId')
};
...
Page.list3Beforedatarender = function(widget, $data) {
    for (var i = 0; i < $data.length; i++) {
        $data[i].nomor = (Page.Variables.vdbTrackingOrder.pagination.number * Page.Variables.vdbTrackingOrder.pagination.size) + (i + 1);
    }
};
...
Page.BindingCompany = function(code) {
    if (code == 1000) {
        return "PT. Amerta Indah Otsuka"
    } else if (code == 2000) {
        return "PT. Otsuka Distribusi Indonesia"
    }
};


Page.vdbTrackingOrderonSuccess = function(variable, data) {
    console.log(data)
};
...
Page.container45_1Click = function($event, widget) {
    Page.toggleTableSort('deliveryDate')
};
Page.container47Click = function($event, widget) {
    Page.toggleTableSort('paidOn')
};
Page.container49Click = function($event, widget) {
    Page.toggleTableSort('status')
};
Page.picture14_2Click = function($event, widget, item, currentItemWidgets) {
    // console.log(item)
    Page.Actions.gotoDetailPO.invoke({
        data: {
            'company_code': item.companyId,
            'id_po': item.idPoHeader
        }
    });
};
